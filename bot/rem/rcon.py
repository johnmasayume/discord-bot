...
class SourceRcon:
    ...
    def create_packet(
        ...
        logging.debug(f"Final packet: {final_packet}")
        return final_packet
    ...
    def check_auth_response(self, auth_response_packet: bytes) -> bool:
        ...
        if (
            ...
            logging.error("Invalid response or wrong packet type.")
            return False
    ...
    def auth_to_rcon(self, socket: socket.socket) -> bool:
        ...
        logging.debug("Authenticating to server rcon before sending command.")
    ...
    def establish_connection(self, socket: socket.socket) -> bool:
        try:
            ...
            logging.error(f"Error while establishing a connection: {e}")
            return False
    ...
    def execute_command(self, socket: socket.socket, command: str) -> str:
        ...
        logging.debug(f"Command response: {unpacked_packet.body}")
